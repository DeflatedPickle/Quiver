...
object ImageViewerPlugin {
    ...
    private fun newAnimationThread() = Thread {
        while (shouldAnimate) {
            if (ImageViewer.index >= ImageViewer.maxIndex) {
    ...
    private val playButton: JToggleButton = JToggleButton(NagatoIcon.RUN).apply {
        addItemListener {
            when (it.stateChange) {
                ItemEvent.SELECTED -> {
                    if (!loopButton.isSelected && ImageViewer.index == ImageViewer.maxIndex) {
                        ImageViewer.index = 0
                    }
    ...
    private val previousButton = JXButton(NagatoIcon.ARROW_LEFT).apply {
        addActionListener {
            if (loopButton.isSelected && ImageViewer.index <= 0) {
                ImageViewer.index = ImageViewer.maxIndex
            } else {
                ImageViewer.index--
            }
            Quiver.selectedFile?.let { ImageViewer.refresh(it) }
